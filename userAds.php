<?php
/*
This is the user advertisement page that generates a table listing all of the ads
the user has created along with its status. This web page also shows ads that are
rejected.
...
<!-- ============ MAIN CONTENT ============== -->
	<td width="80%" valign="top" bgcolor="#d2d8c7">
		...
		<?php
		/*Used to output total ads posted by the user regardless of the ads status.*/
		//Database information is retrieved
			...
			$output2 = $db->query("select COUNT(*) from `tlaadvertisement` where user_name = '".$userNam."'");
		...
		?>
		ads that YOU have posted</div>
		<!-- Generate advertisment listing of all ads posted by the user -->
...
try {
	//Database information is retrieved
	  ...
	  //Allows user to set stored advertisements to a 'removed' state, this does not fully delete the adID
		//Admin is only able to fully delete ad
		if(isset($_POST['Remove'])){
		//set state of advertisement to 'removed'
		$db->query("update tlaadvertisement set status = 'Removed' where id = '" .$_POST['adID']. "'");
		}
	  
	//Temporarily holds a string, used to hold description of the advertisment for user
	$tempStoreInfo = "";
	//information is recieved from the database and stored in the $output variable
	$output = $db->query("SELECT * from tlaadvertisement where account_type = '".$accountType."' and user_name ='".$userNam."'
	and status != 'Removed' ORDER BY date_Posted DESC");
