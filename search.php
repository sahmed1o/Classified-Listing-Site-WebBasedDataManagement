<?php
/*This web page outputs a table containing data that contain key words specified by the user on
the search bar.
...
<!-- ============ MAIN CONTENT ============== -->
	<td width="80%" valign="top" bgcolor="#d2d8c7">
		...
		<?php
		/*Used to output all ads matchin user keyword search.*/
		//Database information is retrieved
			require_once("web_db.php"); 
			//get the information from the table and store it in the $output2 variable
			$output2 = $db->query("select COUNT(*) from `tlaadvertisement` where status = 'Approved'");
		...
		<!-- Generate advertisment listing of ads matching search input -->
...
try {
	//Database information is retrieved
	 ...
	 //retrieved data from search box
	$getGks = $_GET['gks'];
	$getStreet = $_GET['street'];
	$getProvi = $_GET['provi'];
	$getCity = $_GET['city'];
	$getAcctyp = $_GET['acctyp'];
	$getDatert = $_GET['datert'];
	$getEprice = $_GET['eprice'];
	$getPricer = $_GET['pricer'];
	
	//information is recieved from the database and stored in the $output variable
	$output = $db->query("SELECT * from tlaadvertisement where Street REGEXP '[[:<:]]".$getStreet."[[:>:]]' 
	OR advertisement_title REGEXP '[[:<:]]".$getGks."[[:>:]]' OR province REGEXP '[[:<:]]".$getProvi."[[:>:]]' 
	OR city REGEXP '[[:<:]]".$getCity."[[:>:]]' OR account_type REGEXP '[[:<:]]".$getAcctyp."[[:>:]]'
	OR date_Posted REGEXP '[[:<:]]".$getDatert."[[:>:]]' OR price REGEXP '[[:<:]]".$getEprice."[[:>:]]'
